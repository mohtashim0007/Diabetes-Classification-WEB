...
if radio == 'Use Testing Data':
  ...
  with col1:
    Glucose = st.number_input('Glucose :',  df['Glucose'].min(), df['Glucose'].max(), value = df['Glucose'].mean())
    Glucose = (Glucose - df['Glucose'].min()) / (df['Glucose'].max() - df['Glucose'].min())
    BloodPressure = st.number_input('BloodPressure :',  df['BloodPressure'].min(), df['BloodPressure'].max(), value = df['BloodPressure'].mean())
    BloodPressure = (BloodPressure - df['BloodPressure'].min()) / (df['BloodPressure'].max() - df['BloodPressure'].min())
    SkinThickness = st.number_input('SkinThickness :',  df['SkinThickness'].min(), df['SkinThickness'].max(), value = df['SkinThickness'].mean())
    SkinThickness = (SkinThickness - df['SkinThickness'].min()) / (df['SkinThickness'].max() - df['SkinThickness'].min())
    Insulin = st.number_input('Insulin :',  df['Insulin'].min(), df['Insulin'].max(), value = df['Insulin'].mean())
    Insulin = (Insulin - df['Insulin'].min()) / (df['Insulin'].max() - df['Insulin'].min())

  with col2:
    BMI = st.number_input('BMI :',  df['BMI'].min(), df['BMI'].max(), value = df['BMI'].mean())
    BMI = (BMI - df['BMI'].min()) / (df['BMI'].max() - df['BMI'].min())
    Age = st.number_input('Age :',  df['Age'].min(), df['Age'].max(), value = 27)
    Age = (Age - df['Age'].min()) / (df['Age'].max() - df['Age'].min())
    DiabetesPedigreeFunction = st.number_input('DiabetesPedigreeFunction :',  df['DiabetesPedigreeFunction'].min(), df['DiabetesPedigreeFunction'].max(), value = df['DiabetesPedigreeFunction'].mean())
    ...
    Pregnancies = st.number_input('Pregnancies :',  df['Pregnancies'].min(), 9, value = 0)
